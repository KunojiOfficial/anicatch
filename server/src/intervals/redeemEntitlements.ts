...
export default async function redeemEntitlements(db: PrismaClient, manager: ClusterManager) {
    ...
    setInterval(async () => {
        ...
        const subscriptions = entitlements.filter(e => e.type === EntitlementType.ApplicationSubscription);
